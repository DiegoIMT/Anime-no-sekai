...
function bindProductFormDirtyTracking(form){
 ...
 form.addEventListener('input',e=>{const el=e.target;if(el?.classList?.contains('fieldInvalid')){el.classList.remove('fieldInvalid');el.closest('label,.catalogField')?.querySelector('.fieldErrorText')?.remove()}});
}
...
function openProductForm(p=null,duplicating=false){
 ...
 <label>Nombre<input name="nombre" maxlength="150" required value="${attr(p?.nombre||'')}"></label><div class="catalogField"><span>Franquicia</span><div class="catalogPicker"><div class="searchableSelect" id="franquiciaCombo"><input class="searchableSelectInput" id="franquiciaSearch" type="text" autocomplete="off" placeholder="Buscar franquicia…"><button class="searchableSelectArrow" type="button" tabindex="-1" aria-label="Mostrar franquicias">⌄</button><div class="searchableSelectMenu"></div><select name="franquicia_id" id="franquiciaSelect" class="catalogNativeSelect"><option value="">Cargando franquicias…</option></select></div><button class="catalogAdd" id="addFranquicia" type="button">＋ Nueva</button></div><small class="fieldHint">Escribe para buscar una franquicia existente o créala sin salir del producto.</small></div>
...
async function saveAdditionalProduct(e,id,currentSku){
 ...
 if(!obj.tipo_producto_id){showProductMessage('Selecciona un tipo de producto.',true);return}const validationErrors=validateProductCommon(obj,obj.activo);if(validationErrors.length){showValidationErrors(e.currentTarget,validationErrors);return}const stateWarning=commercialStateWarning(obj);if(stateWarning&&!confirm(stateWarning))return;
