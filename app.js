...
function renderPublicProducts(){
 ...
 const featured=searched.filter(p=>p.featured);
 document.getElementById('offersGrid').innerHTML=featured.length?featured.map(productCard).join(''):`<div class="catalogEmpty"><b>${query?'No encontramos figuras destacadas con esa búsqueda.':'Aún no hay figuras destacadas.'}</b><span>${query?'Prueba con otro nombre, personaje o franquicia.':'Marca una figura como destacada desde administración para mostrarla aquí.'}</span></div>`;
