...
function openHelpView(push=true,embedded=false){
 ...
 <details id="help-extra"><summary><span>05</span><b>¿Qué es Ami no Sekai?</b></summary><div><p>Es el espacio para productos complementarios como amigurumis, peluches, llaveros, tazas y otros artículos. Algunos pueden ser hechos a mano o estar disponibles sobre pedido; revisa la información indicada en cada producto.</p></div></details>
...
const THEME_STORAGE_KEY='animeNoSekaiTheme';
function currentTheme(){return document.documentElement.dataset.theme==='light'?'light':'dark'}
function syncThemeControl(){
 const theme=currentTheme(),btn=document.getElementById('themeToggle'),meta=document.querySelector('meta[name="theme-color"]');
 if(btn){btn.setAttribute('aria-label',theme==='dark'?'Activar modo claro':'Activar modo oscuro');btn.title=theme==='dark'?'Activar modo claro':'Activar modo oscuro';btn.classList.toggle('isLight',theme==='light')}
 if(meta)meta.setAttribute('content',theme==='light'?'#f5f3f8':'#090a0d');
}
function setTheme(theme,{persist=true}={}){
 const next=theme==='light'?'light':'dark';document.documentElement.dataset.theme=next;if(persist){try{localStorage.setItem(THEME_STORAGE_KEY,next)}catch{}}syncThemeControl();
}
function initThemeControl(){const btn=document.getElementById('themeToggle');syncThemeControl();if(btn&&!btn.dataset.bound){btn.dataset.bound='1';btn.addEventListener('click',()=>setTheme(currentTheme()==='dark'?'light':'dark'))}}
...
function applySiteConfig(c){
 ...
 root.style.setProperty('--brand-primary',c.color_primario||'#8B5CF6');root.style.setProperty('--brand-secondary',c.color_secundario||'#A78BFA');
...
async function loadFeaturedFranchises(){
 ...
 box.querySelectorAll('[data-universe]').forEach(b=>b.onclick=()=>{const name=b.dataset.universe;publicCatalogPage=1;publicQuickFilter='todas';publicCatalogFilters.franquicia=name;publicCatalogFilters.personaje='';navigatePublic('/figuras');});
}
...
async function showSiteContentForm(){
 ...
 <section class="formSection"><div class="formSectionTitle"><span>05</span><div><h2>Colores de Anime no Sekai</h2><p>Personaliza los acentos principales de la marca sin modificar el fondo oscuro.</p></div></div><div class="brandColorGrid"><label>Color principal<div class="colorField"><input name="color_primario" type="color" value="${attr(data.color_primario||'#8B5CF6')}"><code>${escapeHtml(data.color_primario||'#8B5CF6')}</code></div></label><label>Color secundario<div class="colorField"><input name="color_secundario" type="color" value="${attr(data.color_secundario||'#A78BFA')}"><code>${escapeHtml(data.color_secundario||'#A78BFA')}</code></div></label></div></section>
 <section class="formSection amiAdminSection"><div class="formSectionTitle"><span>06</span><div><h2>Ami no Sekai</h2><p>Identidad y presentación de Ami no Sekai.</p></div></div><div class="formGrid"><label>Nombre<input name="ami_nombre" maxlength="100" required value="${attr(data.ami_nombre||'Ami no Sekai')}"></label><label>Eslogan<input name="ami_eslogan" maxlength="150" required value="${attr(data.ami_eslogan||'Tejiendo pequeños mundos')}"></label><label class="full">Descripción<textarea name="ami_descripcion" maxlength="500" rows="3" required>${escapeHtml(data.ami_descripcion||'Creaciones artesanales con personalidad propia, hechas para acompañar tu colección.')}</textarea></label><label class="full">Texto de presentación en Inicio<textarea name="ami_texto_inicio" maxlength="300" rows="2">${escapeHtml(data.ami_texto_inicio||'Tejiendo pequeños mundos. Amigurumis y creaciones artesanales hechas con cariño.')}</textarea></label></div><div class="siteVisualGrid amiVisualAdmin"><div class="siteVisualField"><div class="siteVisualLabel"><b>Logo de Ami no Sekai</b><small>PNG, JPG o WebP. Si se quita, se usa el logo incluido en el sitio.</small></div><div class="siteVisualPreview amiLogoPreview" id="amiLogoPreview"><img src="${attr(data.ami_logo_url||'/assets/ami-no-sekai-logo.webp')}" alt="Logo Ami no Sekai"></div><input id="amiLogoInput" type="file" accept="image/jpeg,image/png,image/webp" hidden><div class="siteVisualActions"><button id="chooseAmiLogo" class="secondary" type="button">${data.ami_logo_url?'Cambiar logo':'Subir logo'}</button>${data.ami_logo_url?'<button id="removeAmiLogo" class="visualRemove" type="button">Usar logo predeterminado</button>':''}</div></div><div class="brandColorGrid"><label>Color principal Ami<div class="colorField"><input name="ami_color_primario" type="color" value="${attr(data.ami_color_primario||'#ff78b2')}"><code>${escapeHtml(data.ami_color_primario||'#ff78b2')}</code></div></label><label>Color secundario Ami<div class="colorField"><input name="ami_color_secundario" type="color" value="${attr(data.ami_color_secundario||'#b88cff')}"><code>${escapeHtml(data.ami_color_secundario||'#b88cff')}</code></div></label><label class="siteLogoMode"><span><b>Mostrar Ami no Sekai</b><small>Controla su acceso desde la navegación pública.</small></span><input name="ami_visible" type="checkbox" ${data.ami_visible!==false?'checked':''}></label><label class="siteLogoMode"><span><b>Mostrar en Inicio</b><small>Muestra el bloque resumido de Ami no Sekai en la portada.</small></span><input name="ami_mostrar_inicio" type="checkbox" ${data.ami_mostrar_inicio!==false?'checked':''}></label></div></div></section>
 <section class="formSection"><div class="formSectionTitle"><span>07</span><div><h2>Franquicias destacadas</h2><p>Selecciona los universos que aparecerán en la portada y carga el logo de cada anime. Máximo 8.</p></div></div><div class="featuredFranchiseAdmin" id="featuredFranchiseAdmin">${siteFranchises.length?siteFranchises.map(x=>`<div class="featuredFranchiseOption" data-franchise-row="${attr(x.id)}"><label class="featuredFranchiseCheck"><span class="adminSwitch"><input type="checkbox" name="franquicias_destacadas" value="${attr(x.id)}" ${x.destacada?'checked':''}><span class="adminSwitchTrack"></span></span><span>${escapeHtml(x.nombre)}</span></label><div class="franchiseLogoPreview" id="franchiseLogoPreview-${attr(x.id)}">${x.logo_url?`<img src="${attr(x.logo_url)}" alt="Logo de ${attr(x.nombre)}">`:'<span>Sin logo</span>'}</div><input class="franchiseLogoInput" data-franchise-logo-input="${attr(x.id)}" type="file" accept="image/jpeg,image/png,image/webp" hidden><div class="franchiseLogoActions"><button class="visualRemove" data-franchise-logo-choose="${attr(x.id)}" type="button">${x.logo_url?'Cambiar logo':'Agregar logo'}</button>${x.logo_url?`<button class="visualRemove" data-franchise-logo-remove="${attr(x.id)}" type="button">Quitar</button>`:''}</div></div>`).join(''):'<p class="fieldHint">No hay franquicias disponibles.</p>'}</div><small class="fieldHint">El logo se utiliza en “Explora por universo”. Recomendado: PNG o WebP con fondo transparente.</small></section>
 ...
 siteLogoFile=null;amiLogoFile=null;siteHeroFile=null;siteHeroMobileFile=null;franchiseLogoFiles=new Map();
 const logoInput=document.getElementById('siteLogoInput'),amiInput=document.getElementById('amiLogoInput'),heroInput=document.getElementById('siteHeroInput'),heroMobileInput=document.getElementById('siteHeroMobileInput');
 document.getElementById('chooseSiteLogo').onclick=()=>logoInput.click();
 document.getElementById('chooseAmiLogo')?.addEventListener('click',()=>amiInput?.click());
 ...
 if(amiInput)amiInput.onchange=()=>previewAmiLogo(amiInput.files?.[0]);
 ...
 document.getElementById('removeAmiLogo')?.addEventListener('click',markAmiLogoRemoved);
 document.querySelectorAll('.colorField input[type="color"]').forEach(i=>i.addEventListener('input',()=>{const c=i.parentElement.querySelector('code');if(c)c.textContent=i.value.toUpperCase()}));
...
function previewAmiLogo(file){
 if(!file)return;if(!/^image\/(jpeg|png|webp)$/.test(file.type)){alert('Selecciona una imagen JPG, PNG o WebP.');return}
 if(amiLogoFile?.preview)URL.revokeObjectURL(amiLogoFile.preview);const url=URL.createObjectURL(file);amiLogoFile={file,preview:url,remove:false};const box=document.getElementById('amiLogoPreview');if(box)box.innerHTML=`<img src="${attr(url)}" alt="Vista previa Ami no Sekai">`;
}
function markAmiLogoRemoved(){if(amiLogoFile?.preview)URL.revokeObjectURL(amiLogoFile.preview);amiLogoFile={file:null,preview:null,remove:true};const box=document.getElementById('amiLogoPreview');if(box)box.innerHTML='<img src="/assets/ami-no-sekai-logo.webp" alt="Logo predeterminado Ami no Sekai">'}
...
async function compressSiteImage(file,kind){
 const bitmap=await createImageBitmap(file),isLogo=kind==='logo'||kind==='ami-logo'||kind==='franchise-logo',max=isLogo?1200:2200,scale=Math.min(1,max/Math.max(bitmap.width,bitmap.height));
...
async function uploadSiteAsset(kind,file){
 const blob=await compressSiteImage(file,kind),path=`${kind==='logo'?'logo':kind==='ami-logo'?'ami/logo':kind==='franchise-logo'?'franquicias/logo':kind==='hero-mobile'?'portada-movil':'portada'}-${crypto.randomUUID()}.webp`;
...
async function saveSiteContent(e){
 ...
 const obj={portada_etiqueta:f.get('portada_etiqueta').trim(),portada_titulo:f.get('portada_titulo').trim(),portada_descripcion:f.get('portada_descripcion').trim(),beneficio_1_titulo:f.get('beneficio_1_titulo').trim(),beneficio_1_descripcion:f.get('beneficio_1_descripcion').trim(),beneficio_2_titulo:f.get('beneficio_2_titulo').trim(),beneficio_2_descripcion:f.get('beneficio_2_descripcion').trim(),beneficio_3_titulo:f.get('beneficio_3_titulo').trim(),beneficio_3_descripcion:f.get('beneficio_3_descripcion').trim(),titulo_ofertas:f.get('titulo_ofertas').trim(),titulo_figuras:f.get('titulo_figuras').trim(),titulo_proximamente:f.get('titulo_proximamente').trim(),color_primario:f.get('color_primario')||'#8B5CF6',color_secundario:f.get('color_secundario')||'#A78BFA',ami_nombre:f.get('ami_nombre').trim(),ami_eslogan:f.get('ami_eslogan').trim(),ami_descripcion:f.get('ami_descripcion').trim(),ami_texto_inicio:f.get('ami_texto_inicio').trim(),ami_color_primario:f.get('ami_color_primario')||'#ff78b2',ami_color_secundario:f.get('ami_color_secundario')||'#b88cff',ami_visible:f.get('ami_visible')==='on',ami_mostrar_inicio:f.get('ami_mostrar_inicio')==='on',mostrar_texto_logo:f.get('mostrar_texto_logo')==='on',portada_posicion_x:Number(f.get('portada_posicion_x')||50),portada_posicion_y:Number(f.get('portada_posicion_y')||50),portada_movil_posicion_x:Number(f.get('portada_movil_posicion_x')||50),portada_movil_posicion_y:Number(f.get('portada_movil_posicion_y')||50),fecha_actualizacion:new Date().toISOString()};
 button.disabled=true;msg.textContent='Guardando…';msg.className='formMessage';const oldLogo=siteConfig?.logo_url||null,oldAmiLogo=siteConfig?.ami_logo_url||null,oldHero=siteConfig?.portada_url||null,oldHeroMobile=siteConfig?.portada_movil_url||null;let uploaded=[];
 try{
  ...
  if(amiLogoFile?.file){msg.textContent='Procesando logo de Ami no Sekai…';const x=await uploadSiteAsset('ami-logo',amiLogoFile.file);uploaded.push(x.path);obj.ami_logo_url=x.url}else if(amiLogoFile?.remove)obj.ami_logo_url=null;
