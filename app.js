...
function initPublicCatalogFilters(){
 ...
 const bind=(id,key)=>document.getElementById(id)?.addEventListener('input',e=>{publicCatalogPage=1;publicCatalogFilters[key]=e.target.value;clearTimeout(e.target.__analyticsTimer);e.target.__analyticsTimer=setTimeout(()=>{const valor=e.target.value.trim();if(valor)registerAnalyticsEvent(key==='busqueda'?'busqueda':'filtro',{contexto:key==='busqueda'?{texto:valor}:{tipo:key,valor}})},650);if(key==='franquicia'){publicCatalogFilters.personaje='';const c=document.getElementById('filterCharacter');if(c)c.value='';updateCharacterOptions()}updateMobileCatalogFilterState();refresh()});
